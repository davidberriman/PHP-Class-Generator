...
# ========================================================================#
	...
	function createClass($tableName, $filename){
		...
		if (file_exists($filename) && !is_writable($filename)) {			
			$errorMessage = "The file exists and is not writable";
		...
		if (!$database->get_connection()) {
			$errorMessage = "ERROR  - Database Connect failed.<br/><br/>
		...
		if ($result = mysqli_query($database->get_connection(), "SHOW COLUMNS FROM {$table}")) {
			...
			if(mysqli_num_rows($result) < 1){
				$errorMessage = "The table columns could not be found for: {$table}";
				trigger_error($errorMessage );
			    return $errorMessage;
			}
		...
		if(!isSet($primaryKey)){
			...
			if(isSet($arrayFieldNames[0]['fieldname'])){
				...
				$errorMessage = "The table primary key column could not be found for: {$table}";
				trigger_error($errorMessage );
			    return $errorMessage;
			}
		}
		
		// past all error checking so now create the file and report if there are errors
		...
		if(!isSet($_POST['tableName']) && !isSet($_POST['updateAll'])){
			$filename = "classes/".$filename;
		}else{
			$filename = "include/classes/".$filename;
		}
		...
		if(!fwrite($myfile, $txt)){
			$errorMessage = "There was an error creating the file";
			trigger_error($errorMessage );
		    return $errorMessage;
		}
